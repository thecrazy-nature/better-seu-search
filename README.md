...
1. 同学输入自然语言问题。
2. AI Query Planner 解析意图、拆 `sub_questions`，生成 `retrieval_keywords`、同义查询、学院、年级、身份等结构化字段。
3. 本地检索用 FTS / LIKE / embedding 粗召回候选，尽量提高召回，不在这一层过度替用户下结论。
4. AI Evidence Judge 逐条判断候选是否真的能回答问题，标记 `direct_answer` / `supporting` / `weak` / `wrong_topic` / `stale`。
5. AI Answer Composer 只基于通过证据裁判的来源生成答案，并列出参考消息源。
6. 硬规则校验会确保 URL 和来源来自数据库，`publish_date` 只使用数据库字段。
7. 没有配置 AI Key 时，会自动使用规则解析、宽召回和抽取式总结，方便先跑通演示。
...
- AI Evidence Judge 证据裁判，过滤弱相关、跑题和过旧候选
...
- 检索候选默认经过 AI Evidence Judge 判断证据是否可用
- 答案默认由 AI Answer Composer 基于通过验证的检索来源生成
- 仍会保留“不得脱离来源回答”的系统规则
...
当前内置的是轻量本地哈希向量，用于打通向量检索的存储和融合链路；后续可替换为真实 embedding 模型。

```powershell
python -m backend.app.backfill_embeddings
```
...
- 做后台同义词、来源权重和 Evidence Judge 样例管理。
